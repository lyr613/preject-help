...
export function _watch_fs() {
    ...
    ipcMain.on('fs_open_http', fs_open_http)
}

function fs_open_http(e: Electron.IpcMainEvent, src: string) {
    shell.openExternal(src)
}

function fs_read_img(e: Electron.IpcMainEvent, src: string, book = null) {
    // if (book_id) {
    const src_base = ''
    // }
    if (src_base) {
